...
export default function AddActionDetailPage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Initialize form data object with default values for each field
        const initialFormData = {};
        response.data.forEach((field) => {
          initialFormData[field.req_name] = "";
        });
  ...
  async function handleSubmit(e) {
    ...
    setIsLoading(true);
    try {
      const response = await api.post(action.post_http_req, formData, {
        headers: {
      ...
      if (error.response.statusText === "Bad Request") {
        ...
        if (error.response.data.message === "unauthorized") {
          setError("no permission to add action detail data");
        } else {
              ...
              <div className="flex flex-col items-center">
                ...
                >
                  ...
                  <button
                    ...
                    disabled={isLoading}
                  >
                    {isLoading ? "Tambah Data..." : "Tambah Data"}
                  </button>
                  {error && (
